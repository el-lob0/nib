...
int init_os_window(sds a, int init_w, int init_h) {
  ...
  Pixel *actual_buffer = init_buffer(RESIZE_ELEMENT.w, RESIZE_ELEMENT.h);

  glfwSetFramebufferSizeCallback(window, frame_resize);

  while (!glfwWindowShouldClose(window)) {

    actual_buffer = init_buffer(RESIZE_ELEMENT.w, RESIZE_ELEMENT.h);
    test_fill((Pixel){0.0f, 0.0f, 1.0f, 1.0f}, actual_buffer, RESIZE_ELEMENT.w,
              RESIZE_ELEMENT.h);
    draw_square((Pixel){0.0f, 1.0f, 0.0f, 1.0f}, actual_buffer,
                RESIZE_ELEMENT.w, RESIZE_ELEMENT.h);

    display_buffer(window, actual_buffer, RESIZE_ELEMENT.w, RESIZE_ELEMENT.h);

    // i dont need to mutate the buffer itself when resizing i just need to send
    // the new size.
...
int main(void) { init_os_window("Le Window", 500, 500); }
